...
st.set_page_config(page_title="AI Multi-Agent Shopping System", page_icon="🛒", layout="wide")
...
def run_agent(agent_id, query, context):
    user_msgs = {
...
.main-title { text-align:center; font-size:2.2rem; font-weight:700; color:#ffffff; margin-bottom:0.3rem; }
.sub-title { text-align:center; color:#888899; font-size:1rem; margin-bottom:1.5rem; }
.badges { display:flex; gap:10px; justify-content:center; flex-wrap:wrap; margin-bottom:2rem; }
.badge { padding:5px 14px; border-radius:20px; font-size:13px; font-weight:600; border:1.5px solid; }
...
.agent-card  { background:#14141c; border:1px solid rgba(255,255,255,0.08); border-radius:12px; padding:1rem; margin-bottom:0.8rem; min-height:80px; }
...
.final-box    { background:#14141c; border:2px solid #4ade80; border-radius:14px; padding:1.5rem; margin-top:1rem; }
...
.metric-box   { background:#14141c; border:1px solid rgba(255,255,255,0.08); border-radius:10px; padding:1rem; text-align:center; }
...
<div class="badges">
  ...
  <span class="badge badge-green">⚡ Groq LLaMA 3</span>
...
for i, agent in enumerate(AGENTS):
    with cols[i % 2]:
        ...
        agent_slots[agent["id"]].markdown(
            f'<div class="agent-card"><div class="agent-name">{agent["name"]}</div><div class="agent-output" style="color:#333344">⏳ Waiting...</div></div>',
            unsafe_allow_html=True)
...
if run and query:
    ...
    for agent in AGENTS:
        agent_slots[agent["id"]].markdown(
            f'<div class="agent-card running"><div class="agent-name">{agent["name"]} <span style="color:#4f9eff;font-size:11px">● RUNNING</span></div><div class="agent-output" style="color:#4f9eff">🔄 Analyzing...</div></div>',
            unsafe_allow_html=True)
        ...
        try:
            ...
            agent_slots[agent["id"]].markdown(
                f'<div class="agent-card done"><div class="agent-name">{agent["name"]} <span style="color:#4ade80;font-size:11px">✓ DONE</span></div><div class="agent-output">{output}</div></div>',
                unsafe_allow_html=True)
            ...
            agent_slots[agent["id"]].markdown(
                f'<div class="agent-card error"><div class="agent-name">{agent["name"]} <span style="color:#f87171;font-size:11px">✗ ERROR</span></div><div class="agent-output" style="color:#f87171">{str(e)}</div></div>',
                unsafe_allow_html=True)
